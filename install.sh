...
function install_prerequisites() {
	...
	if [[ "$PKG_MGR" == "yum" ]]; then
		sudo "$PKG_MGR" install git jq golang bat btop python-lsp-server lsd tmux thefuck -y
    sudo yum install -y https://dl.fedoraproject.org/pub/epel/epel-release-latest-8.noarch.rpm
	elif [[ "$PKG_MGR" == "apt-get" ]]; then
		sudo "$PKG_MGR" install git jq golang bat btop python3-pylsp lsd tmux thefuck -y
	fi
